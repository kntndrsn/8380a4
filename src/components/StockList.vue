    ...
    <v-container fluid>
      ...
      <v-row align="center" justify="center">
        <v-col cols="12" md="10" v-resize="onResize">
          ...
          >
            <template v-slot:item="props">
              <tr>
                ...
                <td nowrap="true" align="left">
                  {{ props.item.purchase_date }}
                </td>
                <td align="center">
                  <v-icon @click="updateStock(props.item)">mdi-pencil</v-icon>
                </td>
                <td align="center">
                  <v-icon @click="deleteStock(props.item)">mdi-delete</v-icon>
                </td>
              </tr>
            </template>
          </v-data-table>
          <v-data-iterator :items="stocks" hide-default-footer v-else>
                  ...
                  <v-card>
                    <v-card-title class="pb-0 pt-0 pl-0">
                      ...
                      <v-col cols="3" class="text-center">
                        <v-card-actions>
                          <v-icon @click="updateStock(item)" class="small"
                            >mdi-pencil</v-icon
                          >
                          <v-icon @click="deleteStock(item)" class="small"
                            >mdi-delete</v-icon
                          >
                          <v-icon
                    ...
                    <v-list v-show="isExpanded(item)" dense>
                      ...
                      <v-list-item>
                        <v-list-item-content
                          >Purchase Price:</v-list-item-content
                        >
                        <v-list-item-content class="align-end"
                          >{{ item.purchase_price }} on
                          {{ item.purchase_date }}</v-list-item-content
                        >
                      </v-list-item>
                    </v-list>
                  </v-card>
          ...
          <v-btn class="blue mt-4 white--text" @click="addNewStock"
            >Add Stock</v-btn
          >
        </v-col>
      </v-row>
